...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $this->grantPermissions('clinic_manager', ['view', 'add', 'update'], 'clinic');
    }
        ...
        foreach (['saas', 'clinic'] as $panel) {
            Permission::query()
                ->whereIn('name', collect(array_keys(PanelPermissionMatrix::ACTIONS))
                    ->map(fn (string $action): string => PanelPermissionMatrix::permissionName($panel, 'insurance_directory', $action))
                    ->all())
                ->delete();
        }
    }
