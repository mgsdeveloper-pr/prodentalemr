...
class VerificationUnassignedRequests extends Page implements HasTable
{
    ...
    protected static ?string $title = 'Unassigned Requests';

    protected static ?string $slug = 'unassigned-requests';

    protected string $view = 'filament.admin.pages.verification-unassigned-requests';
    ...
    {
        return 'Assign incoming verification requests that do not currently have an owner.';
    }
        ...
        return $table
            ...
            ->actions([
                ...
                Action::make('view')
                    ->label('View')
                    ->color('gray')
                    ->url(fn (BillingWorkItem $record): string => VerificationRequestResource::getUrl('view', ['record' => $record])),
                Action::make('edit')
                    ->label('Edit')
                    ->color('gray')
                    ->url(fn (BillingWorkItem $record): string => VerificationRequestResource::getUrl('edit', ['record' => $record])),
            ])
