...
class VerificationNotificationCentre extends Page
{
    ...
    {
        return auth()->user()?->canAccessClinicModule('verification_requests') ?? false;
    }
    ...
    public function mount(): void
    {
        ...
        VerificationNotificationCenter::syncSlaAlertsForUser(auth()->user(), 'clinic', ClinicPanelScope::selectedClinicId());
    }
    ...
    public static function getNavigationBadge(): ?string
    {
        $count = VerificationNotification::query()
            ...
            ->where('panel', 'clinic')
            ->whereNull('read_at')
            ->when(filled(ClinicPanelScope::selectedClinicId()), fn (Builder $query) => $query->where('clinic_id', ClinicPanelScope::selectedClinicId()))
            ->count();
