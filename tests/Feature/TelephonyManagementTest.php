...
it('waits for MightyCall to be ready before placing an outbound call', function (): void {
    ...
    expect($control)
        ...
        ->toContain('if (readyStatuses.includes(status))')
        ->toContain('phoneStatus(phone = window.MightyCallWebPhone?.Phone)')
        ->toContain("const frameExists = Boolean(document.getElementById('mightyCallWebPhoneFrame'))")
        ->toContain('if (! frameExists)')
        ->toContain('async waitForPhoneBridge()')
        ->toContain("if (status === 'closed')")
