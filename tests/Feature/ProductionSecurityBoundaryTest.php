...
it('uses one compact audited portal credential workspace across clinic and verification panels', function () {
    ...
    expect($clinicView)
        ...
        ->toContain('Select a clinic to view credentials')
        ->toContain('pd-credential-empty-state')
        ->toContain('--pd-credential-page-gutter: clamp(1.25rem, 2vw, 2rem)')
        ->toContain('margin: 0 calc(-1 * var(--pd-credential-page-gutter))')
        ->toContain('margin-inline: calc(-1 * var(--pd-credential-page-gutter))')
