            ...
            <div style="padding: 22px 24px; border-bottom: 1px solid #edf2f7;">
                <div style="display: flex; flex-direction: column; gap: 18px;">
                    ...
                    <p style="margin: 0; max-width: 920px; font-size: 15px; line-height: 1.7; color: #64748b;">
                        Review the portal link, username, and password your clinic will use. Any additions or maintenance are handled from the Verification Settings section only.
                    </p>
            ...
            <div style="padding: 20px 24px 24px;">
                @if ($credentials->isEmpty())
                    <div style="border: 1px dashed #cbd5e1; border-radius: 20px; padding: 24px; background: #f8fafc; font-size: 14px; line-height: 1.7; color: #64748b;">
                        No portal credentials are available for this clinic yet.
                    </div>
                @else
                        ...
                        @foreach ($credentials as $credential)
                            <article style="border: 1px solid #dbe4ee; border-radius: 22px; background: #ffffff; box-shadow: 0 10px 24px rgba(15, 23, 42, 0.05); overflow: hidden; height: 100%;">
                                <div style="padding: 18px 20px; border-bottom: 1px solid #edf2f7; display: flex; align-items: flex-start; justify-content: space-between; gap: 12px;">
                                    ...
                                    <div>
                                        <span style="display: inline-flex; align-items: center; justify-content: center; padding: 6px 10px; border-radius: 999px; border: 1px solid {{ $credential->is_active ? '#86efac' : '#d1d5db' }}; background: {{ $credential->is_active ? '#f0fdf4' : '#f8fafc' }}; color: {{ $credential->is_active ? '#15803d' : '#64748b' }}; font-size: 11px; font-weight: 700;">
                                ...
                                <div style="padding: 18px 20px; display: flex; flex-direction: column; gap: 16px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 14px; border-radius: 16px; border: 1px solid #e2e8f0; background: #f8fafc; min-height: 60px;">
                                            <span id="portal-username-clinic-card-{{ $credential->getKey() }}" data-masked="{{ \App\Models\PortalCredential::maskSecret($credential->username) }}" data-visible="0" style="font-size: 14px; font-weight: 700; color: #334155;">
                                                {{ \App\Models\PortalCredential::maskSecret($credential->username) }}
                                            </span>
                                            <div style="display: inline-flex; align-items: center; gap: 8px;">
                                                <button type="button" onclick="togglePortalSecret('portal-username-clinic-card-{{ $credential->getKey() }}', @js($credential->username), this)" title="View username" style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 12px; border: 1px solid #dbe4ee; background: #ffffff; color: #0f172a;">
                                    ...
                                    <div>
                                        ...
                                        <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 14px; border-radius: 16px; border: 1px solid #e2e8f0; background: #f8fafc; min-height: 60px;">
                                            <span id="portal-password-clinic-card-{{ $credential->getKey() }}" data-masked="{{ \App\Models\PortalCredential::maskSecret($credential->password) }}" data-visible="0" style="font-size: 14px; font-weight: 700; color: #334155;">
                                                {{ \App\Models\PortalCredential::maskSecret($credential->password) }}
                                            </span>
                                            <div style="display: inline-flex; align-items: center; gap: 8px;">
                                                <button type="button" onclick="togglePortalSecret('portal-password-clinic-card-{{ $credential->getKey() }}', @js($credential->password), this)" title="View password" style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 12px; border: 1px solid #dbe4ee; background: #ffffff; color: #0f172a;">
                                                    <x-heroicon-o-eye style="width: 18px; height: 18px;" />
                                                </button>
                    ...
                    <div class="portal-password-modal-grid" style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px;">
                        ...
                        <div style="padding: 12px 14px; border-radius: 14px; border: 1px solid #dbe4ee; background: #f8fafc;">
                            ...
                            <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px;">
                                <span id="portal-username-clinic-modal" data-masked="{{ filled($this->editingCredentialUsername) ? \App\Models\PortalCredential::maskSecret($this->editingCredentialUsername) : '-' }}" data-visible="0" style="font-size: 13px; font-weight: 700; color: #334155; word-break: break-all;">
                                ...
                                @if (filled($this->editingCredentialUsername))
                                    <div style="display: inline-flex; align-items: center; gap: 8px; flex-shrink: 0;">
                                        <button type="button" onclick="togglePortalSecret('portal-username-clinic-modal', @js($this->editingCredentialUsername), this)" title="View username" style="display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; border-radius: 12px; border: 1px solid #dbe4ee; background: #ffffff; color: #0f172a;">
                                            <x-heroicon-o-eye style="width: 18px; height: 18px;" />
                                        </button>
                                    </div>
                                @endif
