<x-filament-panels::page>
    ...
    <div class="pd-credential-workspace">
            ...
            <div class="pd-credential-table-wrap">
                <table class="pd-credential-table">
                    ...
                    <tbody>
                            ...
                            <tr wire:key="portal-credential-{{ $credential->getKey() }}">
                                ...
                                <td>
                                    <div class="pd-secret-field">
                                        <code id="portal-username-{{ $credential->getKey() }}" data-masked="{{ \App\Models\PortalCredential::maskSecret($credential->username) }}" data-visible="0">{{ \App\Models\PortalCredential::maskSecret($credential->username) }}</code>
                                        @if (filled($credential->username) && $this->canUpdatePasswords())
                                            ...
                                            <button type="button" wire:click="copyCredentialSecret({{ $credential->getKey() }}, 'username')" wire:loading.attr="disabled" wire:target="copyCredentialSecret({{ $credential->getKey() }}, 'username')" title="Copy username" aria-label="Copy username for {{ $credential->portal_name }}"><x-heroicon-o-clipboard /></button>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    <div class="pd-secret-field">
                                        <code id="portal-password-{{ $credential->getKey() }}" data-masked="{{ \App\Models\PortalCredential::maskSecret($credential->password) }}" data-visible="0">{{ \App\Models\PortalCredential::maskSecret($credential->password) }}</code>
                                        @if (filled($credential->password) && $this->canUpdatePasswords())
                                            ...
                                            <button type="button" wire:click="copyCredentialSecret({{ $credential->getKey() }}, 'password')" wire:loading.attr="disabled" wire:target="copyCredentialSecret({{ $credential->getKey() }}, 'password')" title="Copy password" aria-label="Copy password for {{ $credential->portal_name }}"><x-heroicon-o-clipboard /></button>
                                        @endif
                                    </div>
                                </td>
                                <td><span class="pd-credential-pill {{ $credential->mfa_required ? 'is-info' : '' }}">{{ $credential->mfa_required ? (\App\Models\PortalCredential::MFA_METHOD_OPTIONS[$credential->mfa_method ?: 'none'] ?? 'Required') : 'Not required' }}</span></td>
                        ...
                        @empty
                            <tr><td colspan="8"><div class="pd-credential-empty"><x-heroicon-o-key /><strong>No portal credentials available</strong><span>No visible credentials have been assigned to this clinic yet.</span></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <script>
        (() => {
            ...
            window.addEventListener('portal-credential-copy', async (event) => {
                const detail = event.detail || {};
                const value = detail.value ?? detail[0]?.value ?? '';
