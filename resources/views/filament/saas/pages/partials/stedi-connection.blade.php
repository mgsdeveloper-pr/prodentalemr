...
<div class="es-toolbar"><h2>Stedi</h2><x-filament::badge color="warning">Live patient submissions disabled</x-filament::badge></div>
...
<div class="es-grid">
    <section class="es-band">
        <h2>{{ $environment === 'sandbox' ? 'Test' : 'Production' }} API key</h2>
        <p>Saved key: <strong>{{ $details['has_key'] ? 'Stored securely' : 'Not configured' }}</strong>. Leave the field blank to retain it.</p>
        <p>Use a Stedi {{ $environment === 'sandbox' ? 'test' : 'production' }} key. The selected environment does not change the key's permissions or mode.</p>
        <form wire:submit="save">
            ...
            <div class="es-toolbar" style="margin-top:20px">
                ...
                <x-filament::button type="button" color="gray" icon="heroicon-o-signal" wire:click="testConnection" wire:confirm="Read one page of Stedi's payer directory using the saved key? No patient information is sent." wire:loading.attr="disabled">Test directory access</x-filament::button>
            </div>
        </form>
    ...
    <section class="es-band">
        ...
        <div class="es-note">Directory access does not confirm eligibility access. Test keys may not support payer-directory requests. No mock or live eligibility transactions are submitted by this page.</div>
    </section>
</div>
<section class="es-band">
